...
// selection function
pcb* selectNextProcessSJF(std::vector<pcb>& processes) {
    // SJF selects shortest burst time
    pcb* nextProcess = nullptr;
    for (std::vector<pcb>::iterator it = processes.begin(); it != processes.end(); ++it) {
        pcb& process = *it;
        if (nextProcess == nullptr || process.getTotalTime() < nextProcess->getTotalTime()) {
            nextProcess = &process;
        }
    }
    return nextProcess;
...
int main(int argc, char** argv) {
    ...
    while (!processes.empty()) {
        ...
        pcb* nextProcess = selectNextProcessSJF(processes);
        loadPCB(nextProcess);
    ...
    std::cout << "PCB Results: SJF" << std::endl;
